...
from superanalyze.analysis.analysis_model import (
    ClaimAnalysis,
    ClaimSupport,
    DocAnalysis,
    RigorAnalysis,
...
from superanalyze.analysis.claim_mapping import TOP_K_RELATED, MappedClaims, RelatedChunks
...
def analyze_claim_support(
    ...
    for i, (chunk_id, _score) in enumerate(relevant_chunks, 1):
...
async def analyze_claims_async(
    ...
    def analysis_labeler(i: int, _spec: Any) -> str:
        if i < len(task_types):
